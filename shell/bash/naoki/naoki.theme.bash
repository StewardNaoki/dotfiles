...
#   echo "$(clock_prompt) "
  ...
  then
    echo "[${bold_cyan}$(clock_prompt) $(clock_char)${normal}]"
  else
    echo "[${bold_cyan}$(clock_prompt)${normal}]"
  fi
...
prompt() {
    ...
    # nice prompt
    case "`id -u`" in
        0) PS1="${TITLEBAR}┌─$(my_ve)$(chroot)$(__naoki_clock)[$my_ps_user@$my_ps_host]$(modern_scm_prompt)$(__my_rvm_ruby_version)[${cyan}\w${normal}]$(is_vim_shell)
└─▪ "
        ;;
        *) PS1="${TITLEBAR}┌─$(my_ve)$(chroot)$(__naoki_clock)[$my_ps_user@$my_ps_host]$(modern_scm_prompt)$(__my_rvm_ruby_version)[${cyan}\w${normal}]$(is_vim_shell)
└─▪ "
        ;;
    esac
}
